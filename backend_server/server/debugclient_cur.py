...
class ClientConnection:
    ...
    def readAll( self ):
        ...
        if( len( ret[0] ) > 0 ):
            ...
            if self.state == "auth":
                self.socket.sendall( '{"cmd":"JOIN_REQ", "data":{"token":"combat"}}\r\n' )

            if self.state == "join":
                self.socket.sendall( '{"cmd":"DBG_VIS", "data":{"match_token":"combat"}}\r\n' )
                self.socket.sendall( '{"cmd":"ABL_REQ", "data":{"abl_id":0,"tgt_pos":[0,0,0],"t":1}}\r\n' )
                self.socket.sendall( '{"cmd":"BUILD_ACT_REQ", "data":{"s_id":4, "act_id":2}}\r\n' )
                self.state = "game"
    ...
    def __init__(self, host, port):
        ...
        self.socket.sendall( '{"cmd":"AUTH", "data":{"user":"soiha", "password":"bar"}}\r\n' )
